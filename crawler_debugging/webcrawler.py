...
class WebCrawler(object):
    ...
    def crawl(self, initial_url, output_file, display_results=True):
        ...
        if display_results:
            ...
            if not self.errors:
                print("Crawler: No errors found!")
            else:
                print("Crawler: Here are all the complaints found:")
    ...
    def _enqueue(self, url):
        '''Adds a url to the queue of urls to be looked at. Also adds a node for it in the graph.'''
        ...
        # While the number of threads is less than the maximum specified
        if self._number_of_running_threads < self.max_threads:
            # Spawn crawling thread
            # Note that we are incrementing the counter first
            self._number_of_running_threads += 1
            # Call function to start a new thread
    ...
    def _finalize_crawl(self, url):
        '''
        Logs, removes the URL from the cbe set.
        Checks queue for an entry and then spawns a new thread
        '''
        # Prints that finalize called
        self.logger.finalize_crawl(url)
        self.currently_being_explored.remove(url)
        self.logger.print_cbe_set(self.currently_being_explored)

        if self.queue:
            # if there are still elements in the queue
            url = self.queue.pop()
            # get them first and then start a new thread
            self._spawn_crawling_thread(url)
        else:
            # If nothing new to do then stop thread
            self._number_of_running_threads -= 1
    ...
    def _spawn_crawling_thread(self, url):
        ...
        # This is the line which starts a new thread
    ...
    def url_should_be_crawled_as_node(self, url):
        ...
        prefix_sets = set()
        prefix_sets.add(url_tuple[0])
        prefix_sets.add(domain_tuple[0])
        if (len(prefix_sets) > 1 ) and (prefix_sets != set(['http', 'https'])):
            return False
    ...
    def _crawl_with_get_request(self, url):
        ...
        # The following step allows it to try new things
